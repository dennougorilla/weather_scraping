import datetime, pandas as pd
for year in range(2016, 2019):
    for month in range(1,13):

        if year == 2016 and month < 3:
            continue
        elif year == 2018 and month > 6:
            break

        url = 'http://www.data.jma.go.jp/obd/stats/etrn/view/daily_s1.php?prec_no=36&block_no=47570&year=' +str(year) + '&month=' + str(month) + '&day=01&view=p1'
        df = pd.read_html(url, 
                skiprows=4)[0]

        print(year)
        print(month)
        if year == 2016 and month == 3:
            df = df.iloc[29:,:19]
        elif year == 2018 and month ==6:
            df = df.iloc[:17,:19]
        else:
            df = df.iloc[:,:19]
        df.columns = ['日', '気圧(hPa)現地平均', '気圧(hPa)海面平均', '降水量(mm)合計', '降水量(mm)最大一時間', '降水量(mm)最大１０分間', '気温(℃)平均', '気温(℃)最高', '気温(℃)最低', '湿度(%)平均', '湿度(%)最小', '平均風速(m/s)', '最大風速風速(m/s)', '最大風速風向', '最大瞬間風速風速(m/s)', '最大瞬間風速風向', '日照時間(h)', '雪降雪合計(cm)', '雪最深積雪値(cm)']
        df = df.replace('--', '-1')
        print(df)
        df.to_csv(path_or_buf = "./Past_weather_data/"+str(year)+'_'+str(month)+".csv", index = False)
